...
public class UserServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		if("deladdress".equals(method)){
			doDelAddress(request,response);
		}
	}
	...
	private void doDelAddress(HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		
		String addressid = request.getParameter("addressid");
		String userid = request.getSession().getAttribute("userid")+"";
		int n = userDAO.deladdress(addressid);
		if(n>0){
			List<Address> list = userDAO.FindAddress(userid);
			Map map = new HashMap();
			map.put("list", list);
	...
	private void doAddArea(HttpServletRequest request,
		...
		int n = userDAO.addarea(userid,username,userphone,province,city,area,zipcode,detail);
		if(n>0){
			List<Address> list = userDAO.FindAddress(userid);
			Map map = new HashMap();
			map.put("list", list);
...
//		response.sendRedirect("success");
	...
	private void doFindStatus(HttpServletRequest request,
		...
		int count1 = userDAO.selectorder1(unfinish,userid);
		
		int count2 = userDAO.selectorder2(finish,userid);
