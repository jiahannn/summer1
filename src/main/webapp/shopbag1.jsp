							...
							<c:forEach items="${map.list }" var="goods">
								<tr>
									<td class="td_check" style="width: 57px; height: 263px;">
										<input type="checkbox" name="xz" class="abc" value="${goods.goods_id }"/>
									</td>
									<td class="td_img">
										<img class="img_p" id="img1${goods.goods_id }" src="img/${goods.goods_picture }" />
									</td>
									<td class="td_goods">
										<span class="s1" id="s1${goods.goods_id }">${goods.goods_name }</span>
										<span class="s2" id="s2${goods.goods_id }">${goods.goods_color }</span> 
										<span id="sg${goods.goods_id }" style="display: none;">${goods.goods_id }</span>
										<button class="showModel" onclick="update(${goods.goods_id })">修改</button> 
										<a href="goods.do?method=deleteshopcar&goodsid=${goods.goods_id }" class="delete_fromcar" id="del${goods.goods_id }" onclick="del(${goods.goods_id })">删除</a>
									</td>
									<td class="td_price" id="dj${goods.goods_id }">￥${goods.goods_price }</td>
									<td class="td_num">
										<button class="jian_1" onclick="change('-',${goods.goods_id })">-</button>
										 <input type="text" class="in_num" id="inum${goods.goods_id }" value="${goods.number }" />
										<button class="jia_1" onclick="change('+',${goods.goods_id })">+</button>
									</td>
									<td class="td_all_price" id="price${goods.goods_id }">￥${goods.goods_price * goods.number}</td>
								</tr>
								
								<form action="" method="post">
									<div class="modal" id="modal${goods.goods_id }">
										<div class="modal-content">
											...
											<div class="modal-body">
												<div class="collect_left_img">
													<img class="img_p1" id="img_p1${goods.goods_id }" src="" />
												</div>
												<div class="collect_right_data">
													...
													<div class="add_to_car">
														<div class="choose_g">
															...
															<div class="add_car">
																<p class="goods_price">${goods.goods_price }</p>
...
function update(id){
	...
	function change(op,goodsid){
		...
		if(op=="+"){
			$.ajax({
				  type: "POST",  
				  url: "goods.do?method=changenum&op=jia&goodsid="+goodsid,  
				  contentType: "application/x-www-form-urlencoded;charset=utf-8",  
				  success: function(data){ 
				//	alert(data);
		...
		if(op=="-"){
			if(num>1){
				$.ajax({
					  type: "POST",  
					  url: "goods.do?method=changenum&op=jian&goodsid="+goodsid,  
					  contentType: "application/x-www-form-urlencoded;charset=utf-8",  
					  success: function(data){ 
				//		$("#inum"+id).val() = num*1 - 1 ;
					 } 
...
/*	
	$("#pay").click(function(){
		...
		var arrs = new Array();
		var arrsnum = new Array();
		var arrsprice = new Array();
		$("input[name='xz']:checkbox").each(function(){
			if($(this).prop("checked")){
				arrs.push($(this).val());
				var nums = $(this).parents('tr').children(".td_num").children(".in_num").val();
				var price = $(this).parents('tr').children(".td_all_price").html().substring(1);
				arrsnum.push(nums);
				arrsprice.push(price);
			}
		});
		...
		alert(arrsprice);
		if(sumprice>0){
			location = "pay.do?method=pay&sumprice="+sumprice+"&goodsids="+arrs.join(",")+"&nums="+arrsnum.join(",")+"&price="+arrsprice.join(",");
		}
	...
	$("#pay").click(function(){
		...
		$("input[name='xz']:checkbox").each(function(){
			if($(this).prop("checked")){
				var goodsnum = $(this).parents('tr').children(".td_num").children(".in_num").val();
				var goodsprice = $(this).parents('tr').children(".td_all_price").html().substring(1);
				var goodsid = $(this).val();
				var goodspic = $(this).parents('tr').children(".td_img").children(".img_p").attr("src");
				var goodsname = $(this).parents('tr').children(".td_goods").children(".s1").html();
				var goodscolor = $(this).parents('tr').children(".td_goods").children(".s2").html();
				list.push({"goodsnum":goodsnum,"goodsprice":goodsprice,"goodsid":goodsid,"goodspic":goodspic,"goodsname":goodsname,"goodscolor":goodscolor});
		...
		$.ajax({
			type: "POST",  
			url: "pay.do?method=pay&sumprice="+sumprice,
			contentType: "application/x-www-form-urlencoded;charset=utf-8",
		...
		/*
		var sumprice = $("#sum_price").html().substring(1);
		$.post("pay.do?method=pay&sumprice="+sumprice,{list:JSON.stringify(list)},function(data){
			location="pay.jsp";
		});
		*/
